defmodule BlockScoutWeb.LayoutViewTest do
  ...
  describe "release_link/1" do
    test "set empty string if no Cybascan version configured" do
      Application.put_env(:block_scout_web, :blockscout_version, nil)

      assert LayoutView.release_link(nil) == ""
    end

    test "set empty string if Cybascan version is empty string" do
  ...
  @supported_chains_pattern ~s([ { "title": "RSK", "url": "https://explorer.cybascan.io/rsk/mainnet", "other?": true }, { "title": "Sokol", "url": "https://explorer.cybascan.io/poa/sokol", "test_net?": true }, { "title": "POA", "url": "https://explorer.cybascan.io/poa/core" }, { "title": "LUKSO L14", "url": "https://explorer.cybascan.io/lukso/l14", "test_net?": true, "hide_in_dropdown?": true } ])
  ...
  describe "other_networks/0" do
    test "get networks list based on env variables" do
      ...
      assert LayoutView.other_networks() == [
               %{
                 title: "POA",
                 url: "https://explorer.cybascan.io/poa/core"
               },
               %{
                 title: "RSK",
                 url: "https://explorer.cybascan.io/rsk/mainnet",
                 other?: true
               },
               %{
                 title: "LUKSO L14",
                 url: "https://explorer.cybascan.io/lukso/l14",
                 test_net?: true,
